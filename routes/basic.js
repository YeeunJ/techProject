...
router.get('/', function(req, res) {
  ...
  const query3 = `select cameraID, substr(originalDate, 1, 10) as date, sum(peopleCNT) as people from cam_image where peopleCNT is not null and strftime('%j', regDate) > strftime('%j', datetime('now', 'localtime'), '-7 days') GROUP BY strftime('%j', originalDate), cameraID order by cameraID, date;`;
  const query4 = `select cameraID, substr(originalDate, 1, 16) as date, sum(peopleCNT) as people from cam_image where peopleCNT is not null and strftime('%j', regDate) > strftime('%j', datetime('now', 'localtime'), '-1 days') GROUP BY strftime('%H', originalDate), cameraID order by cameraID, date;`;
