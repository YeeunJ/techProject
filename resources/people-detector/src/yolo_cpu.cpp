...
Yolo_cpu::Yolo_cpu(const string rootPath) {
    this->MODEL_PATH = rootPath + "/bin/model/yolov3.weights";
    this->CFG_PATH = rootPath + "/bin/model/yolov3.cfg";
    this->CLASSES_PATH = rootPath + "/bin/model/coco.names";
...
int Yolo_cpu::doInference(const string inputImagePath, const string outputImagePath, const int resize, const string roiInfo) {
    ...
    printf("people-detector: detected=%d\n", peopleNum);
    return peopleNum;
}
